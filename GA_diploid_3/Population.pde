class Population{
  Creature[] creatures;
  int[] gene_freqs;                 // lsit of freq of genes
  float[] f_list;                     // list of fitness of each creature
  ...
  void selection(){
    ...
    female_Pool.clear();
    for(int n=0; n<creatures.length; n++){
      float fitness = creatures[n].calc_fitness();
      int fitness_int = round(fitness);
      for(int j=0; j<fitness_int; j++){
